      ...
      <div className="bg-card border border-border p-6 md:p-8 rounded-sm mb-6">
          ...
          <div className="text-left md:text-right w-full md:w-auto">
            ...
            {isOwnProfile && (
              <div className="flex flex-wrap gap-2 md:justify-end">
                ...
                <button
                  onClick={onLogout}
                  className="px-4 py-2 bg-secondary hover:bg-secondary/80 rounded-sm text-sm font-medium flex items-center gap-2 text-red-600"
                >
                  <LogOut size={16} />
                  Logout
                </button>
        ...
        <div className="grid grid-cols-3 gap-3 md:gap-6 pt-6 border-t border-border">
          <div className="text-center p-3 md:p-4 bg-secondary rounded-sm">
            <p className="text-xs md:text-sm text-muted-foreground mb-1">Posts</p>
        ...
        <div className="bg-card border border-border rounded-sm">
          ...
          {showAllClaims && (
            <div className="border-t border-border">
              {allClaims.length === 0 ? (
                <p className="p-6 text-sm text-muted-foreground italic">No posts yet</p>
              ) : (
                  ...
                  {allClaims.map((claim) => (
                    <div key={claim.id} className="p-4 hover:bg-secondary/30 transition-colors">
                      <div className="flex items-start justify-between gap-3 mb-2">
                        <p 
                          className="text-sm line-clamp-2 cursor-pointer hover:text-primary flex-1"
                          onClick={() => navigate(`/posts/${claim.id}`)}
                        >
                          {claim.text}
                        </p>
                      </div>
                ...
                <div className="divide-y divide-border max-h-[500px] overflow-y-auto">
                  {allAnnotations.map((annotation) => (
                    <div 
                      ...
                      onClick={() => navigate(`/posts/${annotation.claim_id}`)}
                    >
                      ...
                      {annotation.claim_preview && (
                        <p className="text-xs text-muted-foreground line-clamp-1">
                          On post: "{annotation.claim_preview}"
                        </p>
                      )}
